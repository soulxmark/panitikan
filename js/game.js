...
// GAME ENGINE
// ═══════════════════════════════════════════════════
let gPuntos=0, gStreak=0, gHigh=0, gQ=0, gTotal=10, gDiff='easy',
    gAnswered=false, gTimer=null, gTimeLeft=30, gTanongs=[];
let gCorrect=0;   // ✅ tracks number of correct answers this game
...
function startFreshGame_core() {
  gPuntos=0; gStreak=0; gQ=0; gAnswered=false; gCorrect=0;  // ✅ reset correct count
  // Restore gameCard structure (endGame replaces it with score screen)
  const gc = document.getElementById('gameCard');
  if (gc) gc.innerHTML = `
    <div class="clue-number" id="clueNumber">PAHIWATIG #1</div>
...
function answer(chosen, correct, btn) {
  ...
  if (isRight) {
    gStreak++;
    gCorrect++;   // ✅ increment correct count
...
function updateDisplay() {
  ...
  sv.textContent = gPuntos; sv.classList.remove('pop'); void sv.offsetWidth; sv.classList.add('pop');
...
function endGame() {
  clearInterval(gTimer);
  const gc = document.getElementById('gameCard');
  // ✅ Show correct answers on end screen
  const pct     = Math.round((gCorrect / gTotal) * 100);
  const remark  = gPuntos >= 150
    ? '"Ikaw ay tunay na maalam tungkol sa El Filibusterismo!"'
    : gPuntos >= 80
    ? '"Magaling! Matuto pa tungkol sa mga tauhan."'
    : '"Subukan ulit! Marami pa kayong maaaring matutunan."';
  ...
  gc.innerHTML = `
    <div style="text-align:center;padding:1rem">
      ...
      <div style="font-family:'Cinzel Decorative',serif;font-size:3rem;color:var(--gold-light);margin-bottom:.5rem">${gPuntos}</div>
      <div style="font-family:'Cinzel',serif;font-size:.75rem;letter-spacing:.2em;color:var(--gold);margin-bottom:.25rem">TAMANG SAGOT</div>
      <div style="font-size:1.6rem;color:var(--gold-light);margin-bottom:1rem;font-family:'Cinzel Decorative',serif">
        ${gCorrect} <span style="font-size:1rem;opacity:.7">/ ${gTotal}</span>
        <span style="font-size:.85rem;margin-left:.4rem;color:${pct>=70?'#7eca7e':'#e07070'}">(${pct}%)</span>
      </div>
      <div style="font-size:.95rem;color:rgba(245,234,208,.7);font-style:italic">${remark}</div>
    </div>`;
...
function loadHighPuntoss() {
  ...
  if (!scores.length) {
    list.innerHTML = '<li class="hs-item"><span class="hs-name" style="opacity:.5;font-style:italic">Wala pang naitala</span></li>';
    return;
  }
  const medals = ['🥇','🥈','🥉','4️⃣','5️⃣'];
  scores.forEach((s, i) => {
    const li = document.createElement('li'); li.className = 'hs-item';
    // ✅ show correct count in local high scores list
    const correctLabel = s.correct != null ? ` · ✓${s.correct}/${s.total||10}` : '';
    li.innerHTML = `<span class="hs-rank">${medals[i]||i+1}</span><span class="hs-name">${s.date}${correctLabel}</span><span class="hs-score">${s.score}</span>`;
    list.appendChild(li);
  });
